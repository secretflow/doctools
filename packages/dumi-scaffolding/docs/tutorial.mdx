...
![a screenshot of shortly](_assets/shortly.png):target{#step-0-a-basic-wsgi-introduction}
...
[`Map`](routing.mdx#werkzeug.routing.Map "werkzeug.routing.Map") instance and add a bunch of
[`Rule`](routing.mdx#werkzeug.routing.Rule "werkzeug.routing.Rule") objects.  Each rule has a pattern it will
...
[`NotFound`](exceptions.mdx#werkzeug.exceptions.NotFound "werkzeug.exceptions.NotFound") exception, which is an
[`HTTPException`](exceptions.mdx#werkzeug.exceptions.HTTPException "werkzeug.exceptions.HTTPException").  All HTTP exceptions are also
...
In this case we will raise a [`NotFound`](exceptions.mdx#werkzeug.exceptions.NotFound "werkzeug.exceptions.NotFound") exception
...
the click count to [`int`](https://docs.python.org/3/library/functions.html#int "(in Python v3.11)") by hand.
