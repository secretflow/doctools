...
<Outline domain="py" objectType="class" target={"werkzeug.middleware.shared_data.SharedDataMiddleware"} module={"werkzeug.middleware.shared_data"} fullname={"SharedDataMiddleware"} parameters={["app", "exports", "disallow", "cache", "cache_timeout", "fallback_mimetype"]} description={"A WSGI middleware which provides static content for development\nenvironments or simple server setups. Its usage is quite simple:"}>
  ...
  <Outline.Content>
    ...
    The contents of the folder <code>./shared</code> will now be available on
    <code>http\://example.com/shared/</code>.  This is pretty useful during development
    ...
    This will then serve the <code>static</code> folder in the <cite>myapplication</cite>
    Python package.

    The optional <cite>disallow</cite> parameter can be a list of [<code>fnmatch()</code>](https://docs.python.org/3/library/fnmatch.html#fnmatch.fnmatch "(in Python v3.11)")
      ...
      <dl>
        ...
        <dd className="field-body">
          - <Outline.ParameterTarget>app</Outline.ParameterTarget> (<em>WSGIApplication</em>) – the application to wrap.  If you don’t want to wrap an
            application you can pass it <code>NotFound</code>.
          ...
          - <Outline.ParameterTarget>disallow</Outline.ParameterTarget> (<em>None</em>) – a list of [<code>fnmatch()</code>](https://docs.python.org/3/library/fnmatch.html#fnmatch.fnmatch "(in Python v3.11)") rules.
    ...
    <Container type="info" title="Version changed">
      <span>Changed in version 1.0: </span>The default <code>fallback\_mimetype</code> is
      <code>application/octet-stream</code>. If a filename looks like a text
      mimetype, the <code>utf-8</code> charset is added to it.
    </Container>

    <Container type="info" title="Version added">
      <span>New in version 0.6: </span>Added <code>fallback\_mimetype</code>.
    </Container>

    <Container type="info" title="Version changed">
      <span>Changed in version 0.5: </span>Added <code>cache\_timeout</code>.
    </Container>
