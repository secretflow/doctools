...
create a WSGI environ dict using the <code>create\_environ()</code> helper:
...
For access to the request data the <code>Request</code> object is much more fun.
...
using the <code>from\_values()</code> method:
...
The files are represented as <code>FileStorage</code> objects which provide
some common operations to work with them.

Request headers can be accessed by using the <code>headers</code>
attribute:
...
<code>status</code> and <code>status\_code</code> and the
...
If headers appear multiple times you can use the <code>getlist()</code>
...
call <code>make\_conditional</code> with the request object.
