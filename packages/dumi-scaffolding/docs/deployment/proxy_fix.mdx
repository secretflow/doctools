:target{#tell-werkzeug-it-is-behind-a-proxy}
...
HTTP servers should set <code>X-Forwarded-</code> headers to pass on the real
values to the application. The application can then be told to trust and
use those values by wrapping it with the
[X-Forwarded-For Proxy Fix](../middleware/proxy_fix.mdx) middleware provided by Werkzeug.
