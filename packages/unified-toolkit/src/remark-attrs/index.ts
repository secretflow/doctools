...
// TODO: make this process any HTML attributes
...
export function remarkAttrs(): Transformer {
  return (tree) => {
    visitParents(
      tree,
      convert<TextDirective>((node): node is TextDirective => {
        if (node.type !== 'textDirective') {
          return false;
        }
        if ((<TextDirective>node).name !== 'target') {
          return false;
        }
        if (typeof (<TextDirective>node).attributes?.['id'] !== 'string') {
          return false;
        }
        return true;
      }),
      (node: TextDirective, ancestors: mdast.Parent[]) => {
        ...
        if (['mdxJsxTextElement', 'mdxJsxFlowElement'].includes(nextNode.type)) {
          if (
            (<MdxJsxFlowElement>nextNode).attributes.some(
              (attr) => attr.type === 'mdxJsxAttribute' && attr.name === 'id',
            )
          ) {
            return;
          }
          (<MdxJsxFlowElement>nextNode).attributes.push({
